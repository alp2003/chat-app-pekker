...
export class SocketManager {
    ...
    private async handleConnectError(err: any): Promise<void> {
    ...
    updateToken(newToken: string): void {
        ...
        if (!newToken && this.authToken) {
            console.log(
                "🔄 Ignoring empty token update, refresh may be in progress"
            );
            return;
        }
        ...
        if (this.socket.connected) {
            ...
            console.log(
                "🔄 Socket not connected, connecting with new token..."
            );
            this.socket.connect();
        }
    }
    ...
    private async handleTokenExpiration(): Promise<void> {
        ...
        if (this.refreshRetryCount >= this.maxRefreshRetries) {
            console.error(
                "❌ Max refresh retries exceeded, calling onAuthExpired"
            );
        ...
        try {
            ...
            if (refreshSuccess) {
                console.log(
                    "✅ Token refresh successful, waiting for new cookie..."
                );
                ...
                if (waited >= maxWait) {
                    console.warn(
                        "⚠️ Timeout waiting for new token after refresh"
                    );
                }
            } else {
                console.error(
